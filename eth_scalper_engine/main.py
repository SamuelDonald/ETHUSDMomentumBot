...
async def run_engine() -> None:
    ...
    trend_filter = EmaTrendFilter(
        ...
        weak_multiplier=SETTINGS.strategy.weak_trend_multiplier,
    )
    ...
    signal_engine = SignalEngine(rr=SETTINGS.strategy.risk_reward, sl_buffer=SETTINGS.strategy.stop_loss_buffer)
        ...
        async for payload in ws_client.stream():
            ...
            if stream_name.endswith("@trade"):
                ...
                signal = signal_engine.generate(
                    ...
                    entry_price=latest_orderbook.mid_price or price,
                ...
                if signal:
                    ...
                    logger.info("Signal rejected by strategy gates")
                ...
                kline = data.get("k", {})
                if kline.get("x"):
                    trend_result = trend_filter.update(float(kline["c"]))
                ...
                if kline.get("x"):
                    ...
                    logger.info("BOS update: %s | Market state: %s", latest_bos, latest_market_state)
